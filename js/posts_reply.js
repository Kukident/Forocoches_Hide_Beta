$(document).ready(function() {
  ...
  /* Drag'n drop stuff */
  $('#vB_Editor_001').on('dragover', add_background_textarea)
  $('#vB_Editor_001').on('dragleave', remove_background_textarea)
  ...
  function add_background_textarea() {
    ...
    $('#vB_Editor_001_textarea').css(css_style)
  }
  ...
  $('#vB_Editor_001').on('drop', function(e) {
  ...
  function upload(file) {
    ...
    xhr.onload = function () {
      if (JSON.parse(xhr.responseText).success) {
        ...
        $('#vB_Editor_001_textarea').val($('#vB_Editor_001_textarea').val() + imgur_link)
  ...
  function upload_image_status_animation(color) {
    document.getElementById("vB_Editor_001_textarea").animate([
      // keyframes
      {background: color},
      {background: 'white'}
    ], {
